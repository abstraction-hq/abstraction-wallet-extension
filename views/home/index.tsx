...
const HomePage: React.FC = () => {
    ...
    const password = useCacheStore((state) => state.cachePassword)
    const credentials = useUserStore((state) => state.credentials)

    const account = useRef<AccountService>()

    const initAccount = async () => {
        if (activeWallet !== null && credentials?.encryptedMnemonic && password) {
            const mnemonic = await decryptMnemonic(credentials.encryptedMnemonic, password)
    ...
    useEffect(() => {
        initAccount()
    }, [activeWallet, credentials, password])
